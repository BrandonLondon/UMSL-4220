...
final class ContactsListModel {
    ...
    // ContactsListModel
    init() {
          let people = ["Billy Jol", "Be Yeeted", "Nunya Bidness", "Brandon London", "bobby bob", "Some Body"]
          for person in people {
              let firstName = person.components(separatedBy: " ")[0]
              let lastName = person.components(separatedBy: " ")[1]
              var contact = Contact.instance()
              contact = contact.copy(withNewValue: firstName, for: InputField.firstName)
              contact = contact.copy(withNewValue: lastName, for: InputField.lastName)
              //contact = contact.copy(withNewValue: "true", for: InputField.emergency)
              add(contact)
          }
      }
